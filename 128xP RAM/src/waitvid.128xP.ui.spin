''
'' VGA display 128xP (quad cog) - user interface
''
''        Author: Marko Lukat
...
'' 20120908: setn now returns mailbox address
''
CON
...
CON
  ...
  dcolour_bg = (driver#dcolour >> 0) & %%0030 | %%3{!Z} ' %%--I- blue is palette index
  
OBJ
  driver: "waitvid.128xP.driver"
...
VAR
  ...
  byte  mode, x, y, colour                              ' mode, cursor position, colour
  word  flag[2]
...
PUB setn(n, addr)
  ...
  link[1] := addr
  link{0} := lookupz(n: $001{scrn}, $003{font}, $009{plte}, 0, $00D{bgnd}, 0, $007{crs1})
  repeat
  while link{0}
...
PUB putc(c)
  ...
  if cext                                               ' colour per position mode
    byte[cext][y * columns + x] := colour               ' update colour
...
'
    if cext
      wordmove(cext, cext + columns, constant((bcnt_raw - columns) / 2))
      wordfill(cext + constant(bcnt - columns), colour * $0101, constant(columns / 2))
  ...
  case flag.byte{0}
    $00: case c                                             
           $00..$01,FF:                            
             if c <> $01                           
               if cext
                 wordfill(cext, colour * $0101, constant(bcnt_raw / 2))
           ...
           other: putc(c)                            
    ESC: case c
           "s": mode |= $20                             ' scroll off (page mode)
           "S": mode &= $DF                             ' scroll on
           "o": mode &= $F8                             ' cursor off
           "O": mode |= $07{underline|flash|on}         ' cursor on
...
PRI sc(c, shift, field)

  colour &= !field
  colour |= (c >> shift) & field
